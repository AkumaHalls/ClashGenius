...
class AdminCog(commands.Cog, name="Painel de Administração Avançado"):
    ...
    async def get_api_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            result = {"status": "ok", "message": "API do Clash of Clans operacional.", "latency_ms": latency}
            if hasattr(self.bot.api_client, 'http'):
                try:
                    result["health_stats"] = self.bot.api_client.http.health_stats
                except Exception:
                    pass
            return result
    ...
    async def get_diagnostics(self) -> Dict[str, Any]:
        ...
        if self.bot.api_client and hasattr(self.bot.api_client, 'http'):
            try:
                health = self.bot.api_client.http.health_stats
            except Exception:
                health = {"error": "health_stats indisponível"}
        return {"api_status": api_status, "recent_logs": log_buffer, "health_stats": health}
    ...
    async def smurf_cleanup(self):
        smurf_cog = self.bot.get_cog("Detetor de Smurfs IA")
        if not smurf_cog or smurf_cog.db is None:
            return {"status": "error", "message": "Módulo offline."}
        try:
            ev_count = await smurf_cog.db.smurf_evidence.count_documents({})
            fp_count = await smurf_cog.db.smurf_evidence.count_documents({"_id": {"$regex": "^learning_fp:"}})
            await smurf_cog.db.smurf_evidence.delete_many({"_id": {"$not": {"$regex": "^learning_fp:"}}})
            await smurf_cog.db.smurf_evidence.delete_many({"_id": {"$regex": "^learning_fp:"}})
            smurf_cog._telemetry_cooldown.clear()
            smurf_cog.last_clan_state.clear()
            smurf_cog.last_war_attacks.clear()
            return {
                "status": "success",
                "message": f"Banco limpo! {ev_count} evidências e {fp_count} registros de aprendizado removidos."
            }
        except Exception as e:
