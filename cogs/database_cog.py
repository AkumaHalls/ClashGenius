    ...
    async def save_war_to_history(self, war_data: Dict[str, Any], war_id: str):
        ...
        try:
            ...
            if war_id:
                ...
                # A lógica para limpar guerras antigas usando MAX_WAR_HISTORY
                count = await war_collection.count_documents({})
                if count > self.MAX_WAR_HISTORY: 
                    # Ordena pelo tempo de término para remover a mais antiga
                    oldest_wars_cursor = war_collection.find().sort("war_data.end_time_iso", 1).limit(count - self.MAX_WAR_HISTORY)
                    async for old_war in oldest_wars_cursor:
                        await war_collection.delete_one({"_id": old_war["_id"]})
                        logger.info(f"Guerra mais antiga ({old_war.get('_id')}) removida do histórico para manter o limite de {self.MAX_WAR_HISTORY}.")
            else:
