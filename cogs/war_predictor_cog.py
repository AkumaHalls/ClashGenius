...
class AdvancedFeatureEngineer:
    ...
    async def extract_all_features(self, war, our_clan, opponent, clan_tag):
        try:
            ...
            all_features = {**basic_features, **temporal_features, **coordination_features, **psychological_features, 'historical_win_rate': historical_win_rate}
            return AdvancedWarFeatures(**all_features)
        except Exception as e:
            self.logger.error(f"Erro na extração de features: {e}", exc_info=True)
            return None

    def _extract_basic_features(self, war, our_clan, opponent):
    ...
    def _extract_temporal_features(self, war, our_clan, opponent):
    ...
    def _extract_coordination_features(self, war, our_clan):
    ...
    def _extract_psychological_features(self, war, our_clan, opponent):
    ...
    def _calculate_attack_efficiency(self, attacks):
    ...
    def _calculate_momentum_indicator(self, our_attacks, opp_attacks):
    ...
    def _calculate_clan_synergy(self, our_attacks):
    ...
    def _calculate_pressure_index(self, war, our_clan, opponent):
...
class EnsembleMLSystem:
    ...
    def train(self, historical_data):
        if len(historical_data) < 10:
            self.logger.warning(f"Dados históricos insuficientes ({len(historical_data)}/10). Usando heurística.")
            self.is_trained = False
            return
        try:
            ...
            y = np.array(y_list)
            if X.shape[0] == 0:
                self.logger.warning("Nenhum dado válido para treinamento.")
                self.is_trained = False
                return
            ...
            self.logger.info(f"Ensemble treinado com {len(historical_data)} guerras.")
        except Exception as e:
    ...
    def predict(self, features):
        ...
        try:
            ...
            for name, model in self.models.items():
                weighted_prediction += self.weights[name] * model.predict(feature_vector_scaled)[0]
            return np.clip(weighted_prediction * 100, 1, 99)
        except Exception as e:
            self.logger.warning(f"Erro na predição, usando heurística: {e}")
            return self._heuristic_prediction(features)
...
class WarPredictionSystemV3:
    ...
    async def predict_war_outcome(self, war, clan_tag):
        ...
        try:
            ...
            if war.state == 'preparation':
                return {"summary_panel": "Análise disponível quando a guerra iniciar.", "probability": 50.0, "confidence": 20.0}
            if war.state == 'warEnded':
                our_stars = our_clan.stars if our_clan else 0
                opp_stars = opponent.stars if opponent else 0
                if our_stars > opp_stars:
                    msg = f"Vitória confirmada! {our_stars} x {opp_stars}"
                elif opp_stars > our_stars:
                    msg = f"Derrota confirmada. {our_stars} x {opp_stars}"
                else:
                    msg = f"Empate técnico. {our_stars} x {opp_stars}"
            ...
            if features is None:
                return {"summary_panel": "Aguardando dados para iniciar inferência...", "probability": 50.0, "confidence": 10.0}
            ...
            return {
                ...
                "summary_panel": summaries['panel'], "summary_discord": summaries['discord'],
                "tactical_insights": tactical_insights, "risk_factors": risk_factors,
            ...
            return {"summary_panel": f"Erro na análise", "probability": 50.0, "confidence": 0.0}
            ...
            async for doc in cursor:
                try:
                    features = AdvancedWarFeatures(
                        ...
                        war_progress_percentage=100.0, historical_win_rate=50.0, unused_member_strength_diff=0.0,
                        momentum_indicator=0.5, clan_synergy_score=0.5, pressure_index=0.0
                    )
    ...
    def _check_definitive_scenarios(self, war, our_clan, opponent):
    ...
    def _generate_qualitative_analysis(self, features, probability):
        confidence = 50.0
        ...
        if features.momentum_indicator > 0.55:
            insights.append(f"ritmo favorável (momentum de {features.momentum_indicator:.2f})")
        elif features.momentum_indicator < 0.45:
            insights.append(f"cadência desacelerou (momentum de {features.momentum_indicator:.2f})")
        if features.clan_synergy_score > 0.6:
            insights.append(f"sinergia de {features.clan_synergy_score*100:.0f}% nas limpezas")
        if features.efficiency_ratio > 1.1:
            insights.append(f"eficiência {(features.efficiency_ratio - 1)*100:.0f}% superior ao oponente")
        risks = []
        if features.attacks_remaining_difference < -1:
            risks.append(f"adversário tem {abs(features.attacks_remaining_difference)} ataques a mais")
        if features.unused_member_strength_diff < -100:
            risks.append("peso bélico inimigo não utilizado é maior")
        if features.pressure_index > 0.65:
            risks.append(f"pressão alta ({features.pressure_index:.1f}/1.0) sobre nossos atacantes")
        return confidence, insights, risks

    def _generate_summaries(self, features, probability, our_clan_name, insights, risks):
        prog = int(features.war_progress_percentage)
        prob = int(probability)
        star_diff = int(features.star_difference)
        parts = []
        if prog < 20:
            parts.append(f"Campo aberto ({prog}%). Projeção: {prob}% de vitória.")
        elif prog > 85:
            parts.append(f"Reta final ({prog}%). Chance calculada: {prob}%.")
        else:
            parts.append(f"Guerra em {prog}%. IA projeta {prob}% de chances.")
        if star_diff > 0:
            parts.append(f"Vantagem de {star_diff} estrelas. ")
        elif star_diff < 0:
            parts.append(f"Déficit de {abs(star_diff)} estrelas. ")
        else:
            parts.append(f"Placar empatado. ")
        if insights:
            parts.append("Destaque: " + insights[0] + ". ")
        if risks:
            parts.append("Alerta: " + risks[0] + ". ")
        final_prob = prob
        if final_prob >= 85:
            parts.append("Cenário consolidado. Administrem a vantagem.")
        elif final_prob >= 65:
            parts.append("Vantagem sob controle. Mantenham a pressão.")
        elif final_prob >= 40:
            parts.append("Guerra aberta. Cada estrela decide.")
        elif final_prob >= 20:
            parts.append("Cenário adverso. Hora de arriscar.")
        else:
            parts.append("Situação crítica. Ataques perfeitos ou nada.")
        texto = ''.join(parts)
        return {"panel": texto, "discord": f"**Projeção**\n{texto}"}
...
class WarPredictorCog(commands.Cog, name="Preditor de Guerra"):
    ...
    def __init__(self, bot: commands.Bot):
        ...
        self.logger = logging.getLogger("war_predictor_cog")
        self.logger.info("WarPredictorCog carregado. Sistema será inicializado em segundo plano.")

    async def cog_load(self):
        self.logger.info("Inicializando WarPredictionSystemV3...")
        await self.system.initialize_system()
        self.logger.info("WarPredictionSystemV3 inicializado com sucesso.")
