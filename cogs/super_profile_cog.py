...
def calculate_rushed_status(player):
    ...
    th_hero_expectations = {
        9: 20, 10: 50, 11: 80, 12: 120, 13: 170, 14: 220, 15: 280, 16: 320
    }
    ...
    current_total = sum(h.level for h in player.heroes if h.village == 'home')
...
class SuperProfileView(View):
    ...
    def create_units_embed(self):
        ...
        # Pets (apenas TH14+)
        pets_txt = ""
        for h in p.heroes: # Pets sao tratados como herois no coc.py as vezes ou tem prop propria dependendo da versao
            # Verificação genérica para pets (nomes comuns)
            if h.name in ["L.A.S.S.I", "Electro Owl", "Mighty Yak", "Unicorn", "Diggy", "Frosty", "Poison Lizard", "Phoenix"]:
                 pets_txt += f"{h.name}: {h.level}\n"
    ...
    def create_upgrades_embed(self):
        ...
        # Cálculo simples de progresso de heróis (nível atual / maximo para o TH ATUAL)
        # Nota: coc.py as vezes nao retorna o maximo exato para o TH, entao estimamos
        total_levels = sum(h.level for h in p.heroes if h.village == 'home')
        # Estimativa grosseira de max niveis totais (exemplo)
        max_levels_est = total_levels + 50 # Apenas visual se nao tivermos dados exatos
        
        bar = get_progress_bar(total_levels, max_levels_est)
        embed.add_field(name="Heróis (Estimado)", value=bar, inline=False)
        
        embed.set_footer(text="Nota: O progresso exato depende de dados da Supercell que nem sempre estão disponíveis na API pública.")
        return embed
