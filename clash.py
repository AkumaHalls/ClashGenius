...
def format_war_time_details(war_obj, time_now_tz):
    details = {"time_key": "N/A", "time_value": "N/A", "time_remaining": "N/A"}
...
async def api_members_handler(request): return web.json_response(await get_cached_web_data("web_clan_members", fetch_clan_members_for_web))
async def api_current_war_details_handler(request): return web.json_response(await get_cached_web_data("web_current_war_details", fetch_current_war_details_for_web))
async def api_war_attacks_remaining_handler(request): return web.json_response(await get_cached_web_data("web_war_attacks_remaining", fetch_war_attacks_remaining_for_web))
async def api_war_log_handler(request): return web.json_response(await get_cached_web_data(f"web_war_log_{request.query.get('limit', 10)}", fetch_war_log_for_web, int(request.query.get("limit", 10))))
async def api_cwl_info_handler(request): return web.json_response(await get_cached_web_data("web_cwl_info", fetch_cwl_info_for_web))
async def api_save_player_note_handler(request):
    ...
    try:
        ...
        if "web_clan_members" in web_api_cache: del web_api_cache["web_clan_members"]
...
async def main():
    ...
    try:
        ...
        if 'web_runner' in locals() and web_runner: await web_runner.cleanup()
        if coc_client: await coc_client.close()
        if events_client: await events_client.close()
        if hasattr(bot, 'db_client'): bot.db_client.close()
