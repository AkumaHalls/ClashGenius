...
BOT_VERSION = "31.2.0-GeniusLib-v4.3.0" 
...
class ClashGeniusBot(commands.Bot):
    ...
    async def setup_hook(self) -> None:
        ...
        try:
            ...
            cog_files = [ 'events_cog', 'tasks_cog', 'database_cog', 'general_cog', 'cwl_planner_cog', 'clan_games_cog', 'war_advisor_cog', 'profile_cog', 'maintenance_cog', 'web_api_cog', 'admin_cog', 'donation_cog', 'slash_cog', 'watchlist_cog', 'smurf_detection_cog', 'capital_cog', 'performance_cog', 'war_predictor_cog' ]
            loaded_cogs_count = 0
...
async def setup_web_server(bot_instance: ClashGeniusBot):
    ...
    if not all(required_cogs):
        missing = [name for name, inst in zip(["WebAPI","DB","Profile","CWL","Maint","Advisor","Admin","Watchlist","Capital"], required_cogs) if inst is None]
    ...
    async def admin_get_status_handler(r):
        session=await get_session(r); is_admin=session.get('admin',False) or bool(session.get('role'))
    ...
    async def admin_auth_middleware(request, handler):
        ...
        if not role and not session.get('admin'): return web.json_response({"status":"unauthorized", "message": "Acesso negado."}, status=403)
    ...
    async def api_admin_smurf_dossier(r): return web.json_response(await admin_cog.get_smurf_dossier(), dumps=lambda v: json.dumps(v, default=str))
    ...
    async def api_auth_login(r):
        ...
        if _db is None:
            _db = getattr(bot_instance, 'mongo', None)
        if _db is None:
    ...
    admin_api_app.router.add_get("/export/clan", api_export_clan_handler);
    admin_api_app.router.add_get("/export/players", api_export_players_handler);
    ...
    async def admin_logout_handler(r):
        session=await get_session(r)
        for k in ['admin','authenticated','username','role','guild_id']: session.pop(k,None)
        return web.HTTPFound('/admin')
    async def admin_toggle_maintenance_handler(r):
        session=await get_session(r)
        role=session.get('role')
        if not role and not session.get('admin'): return web.json_response({"status":"unauthorized"}, status=403)
