...
async def fetch_clan_members_for_web_api() -> Dict[str, Any]:
    ...
    # +++ RETORNA O ERRO PARA O FRONT-END SE ALGO FALHAR +++
    except Exception as e:
        logger.error(f"Erro ao buscar membros do clã para API web: {e}", exc_info=True)
        # Retorna o erro real para o front-end, que é o que você está vendo
        return {"error": str(e)}
...
async def fetch_current_war_details_for_web_api() -> Dict[str, Any]:
    ...
    final_response: Dict[str, Any] = {"war_data": None, "all_attacks": [], "our_clan_members_in_war": [], "opponent_clan_members_in_war": []}
    try:
        ...
        final_response["war_data"] = {
            "status": str(war.state), "type": war_type_coc, "state_description": state_description,
            "clan_name": our_clan_obj.name if our_clan_obj else "N/A", "clan_tag": our_clan_obj.tag if our_clan_obj else "N/A",
            "clan_stars": our_clan_obj.stars if our_clan_obj else 0, "clan_destruction": f"{our_clan_obj.destruction:.2f}%" if our_clan_obj else "0.00%",
            ...
            "opponent_name": opp_clan_obj.name if opp_clan_obj else "N/A", "opponent_tag": opp_clan_obj.tag if opp_clan_obj else "N/A",
            "opponent_stars": opp_clan_obj.stars if opp_clan_obj else 0, "opponent_destruction": f"{opp_clan_obj.destruction:.2f}%" if opp_clan_obj else "0.00%",
            ...
            **time_details_coc, "attacks_per_member": war.attacks_per_member, "team_size": war.team_size, "clan_star_distribution": clan_star_dist,
            "opponent_star_distribution": opp_star_dist, "clan_avg_stars": f"{our_clan_obj.stars/c_atk_count:.2f}" if our_clan_obj and c_atk_count > 0 else "0.00",
            ...
            "opponent_avg_duration": f"{o_total_dur/o_atk_count:.1f}s" if o_atk_count > 0 else "0s", "source": "API CoC Oficial"
        }
    except Exception as e:
...
async def fetch_war_attacks_remaining_for_web_api() -> Dict[str, Any]:
    ...
    if war:
        if war.clan and war.clan.tag == CLAN_TAG:
            our_clan_obj_rem = war.clan; clan_name_rem = our_clan_obj_rem.name
        elif war.opponent and war.opponent.tag == CLAN_TAG:
            our_clan_obj_rem = war.opponent; clan_name_rem = our_clan_obj_rem.name
    if not war or war.state not in ["inWar", "preparation"] or not our_clan_obj_rem:
        message_to_show = "Não há guerra em andamento ou em preparação."
        if war and war.state not in ["inWar", "preparation"]:
            message_to_show = f"A guerra está no estado '{str(war.state)}'. Ataques pendentes são mostrados para 'inWar' ou 'preparation'."
        elif not our_clan_obj_rem and war:
             message_to_show = "Erro ao identificar o clã na guerra para listar ataques pendentes."
        return {"message": message_to_show, "members_pending": [], "clan_name": clan_name_rem}
    pending = []
    if hasattr(our_clan_obj_rem, 'members') and our_clan_obj_rem.members:
        for m in our_clan_obj_rem.members:
            attacks_left = war.attacks_per_member
            if war.state == "inWar": attacks_left = war.attacks_per_member - (len(m.attacks) if m.attacks else 0)
    ...
    message_header = "Membros com ataques pendentes:"
    if war.state == "preparation": message_header = "Ataques disponíveis (Fase de Preparação):"
    return {"message": message_header, "members_pending": pending, "clan_name": clan_name_rem}
...
async def fetch_cwl_info_for_web_api() -> Dict[str, Any]:
    ...
    try:
        ...
        season_str = lg.season if hasattr(lg, 'season') and lg.season else "N/A"
        return {"status":"InCwl", "state": str(lg.state), "season": season_str, "clans_in_group":clans_data, "rounds":rounds_data}
...
async def api_get_player_note_handler(request: web.Request) -> web.Response:
    ...
    note_info = note_doc.get("notes", {"text": "", "priority": "none"}) if note_doc else {"text": "", "priority": "none"}
...
async def api_save_player_note_handler(request: web.Request) -> web.Response:
    ...
    try:
        ...
        await bot.db.players.update_one(
            {'_id': player_tag_fmt},
            {'$set': {'name': player_name, 'notes.text': note_text, 'notes.priority': note_priority, 'last_updated': datetime.datetime.now(pytz.utc)}},
            upsert=True
        )
        logger.info(f"Nota salva no DB para {player_tag_fmt}: Prio: {note_priority}")
    ...
    except Exception as e: logger.error(f"Erro ao salvar nota para {player_tag_fmt} no DB: {e}", exc_info=True); return web.json_response({"error": "Erro interno ao salvar nota"}, status=500)
...
async def on_app_command_error(interaction: discord.Interaction, error: app_commands.AppCommandError):
    # Esta função pode ser resumida para economizar espaço, já que não foi alterada
    pass
...
async def setup_hook():
    ...
    logger.info("Sincronizando comandos de app no setup_hook...");
    try:
        synced_cmds = await bot.tree.sync()
        logger.info(f"{len(synced_cmds)} comandos (/) globais sincronizados.")
    except Exception as e: logger.error(f"Erro ao sincronizar comandos globais (/): {e}", exc_info=True)
...
async def main():
    ...
    async with bot:
        try:
            ...
            if hasattr(bot, "web_runner") and bot.web_runner is not None: await bot.web_runner.cleanup()
            if hasattr(bot, "coc_client") and bot.coc_client.http and not bot.coc_client.http.closed : await bot.coc_client.close()
            if hasattr(bot, "db_client") and bot.db_client is not None: bot.db_client.close()
...
if __name__ == "__main__":
    required_env_vars = ["DISCORD_TOKEN", "COC_EMAIL", "COC_PASSWORD", "CLAN_TAG", "CHANNEL_ID", "MONGO_DB_URL"]
    missing_vars = [var for var in required_env_vars if not os.getenv(var)]
    if missing_vars:
        logger.critical(f"Variáveis de ambiente essenciais faltando: {', '.join(missing_vars)}. Verifique seu arquivo .env ou as configurações do Render.com.")
        exit(1)
    ...
    except KeyboardInterrupt: logger.info("Bot interrompido pelo usuário.")
    finally:
        logger.info("Finalizando o loop de eventos...")
